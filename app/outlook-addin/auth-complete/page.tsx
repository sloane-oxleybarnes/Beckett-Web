...
export default function OutlookAuthCompletePage() {
  ...
  useEffect(() => {
    const finish = async () => {
      ...
      if (!accessToken) return setMessage("We could not confirm your sign-in. Close this window and try again.");
      const send = () => { window.Office?.context?.ui?.messageParent?.(JSON.stringify({ type: "beckett-auth-success", accessToken, email: data.session?.user.email || null })); setMessage("Connected. This window will close automatically."); };
