...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from("workday_reminders").update({ active: body.active, updated_at: new Date().toISOString() }).eq("id", (await params).id).eq("user_id", user.id).select("*").single();
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return NextResponse.json({ error: "Unauthorized." }, { status: 401 });
  const { error } = await supabase.from("workday_reminders").delete().eq("id", (await params).id).eq("user_id", user.id);
