...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => null) as { action?: unknown; text?: unknown; context?: unknown; person?: unknown; goal?: unknown; warmth?: unknown; directness?: unknown; formality?: unknown; length?: unknown } | null;
  ...
  const safety = getSafetyResponse([text, context, goal].filter(Boolean).join("\n"), profile?.safety_resource_region);
