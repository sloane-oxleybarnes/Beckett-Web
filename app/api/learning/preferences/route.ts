...
export async function GET() {
  ...
  const { data, error } = await supabaseAdmin.from("profiles").select(fields).eq("id", user.id).maybeSingle();
...
export async function PATCH(request: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from("profiles").update({ ...update, updated_at: new Date().toISOString() }).eq("id", user.id).select(fields).single();
