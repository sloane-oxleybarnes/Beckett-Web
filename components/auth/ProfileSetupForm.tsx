...
const workAppOptions: Array<{
  ...
  },
  {
    id: "outlook",
    label: "Outlook",
    description: "Decode one selected received message at a time from Beckett's read-only Outlook task pane.",
    availability: "available",
    walkthrough: [
      "Install Beckett from Microsoft Marketplace and open a received Outlook message.",
      "Open Beckett from the message command bar, sign in to Beckett, and choose Decode.",
      "The Marketplace add-in is read-only. A separate Microsoft 365 calendar connection is optional and is not required for Outlook message coaching.",
    ],
  },
  {
    id: "chrome",
                  ...
                  {workAppOptions.filter((option) => workApps.includes(option.id)).map((option) => (
                    <div key={option.id} className="rounded-sm border border-border bg-bg/60 p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {option.id === "outlook" && <button type="button" onClick={() => completeOnboarding()} disabled={saving} className="shrink-0 rounded-pill border border-primary/30 px-3 py-2 text-xs font-medium text-primary hover:bg-primary-light disabled:opacity-50">Finish setup</button>}
