    ...
    <div className="mx-auto max-w-5xl pb-12">
      ...
      {response.length === 0 ? <>
        <form onSubmit={submit} className="rounded-card border border-border bg-white p-5 shadow-sm sm:p-6">
          ...
          <label className="mt-6 block text-sm font-medium text-ink">{inputLabel[selectedActions[0]]}<span className="relative mt-2 block"><textarea value={text} onChange={(event) => setText(event.target.value)} maxLength={5000} rows={9} placeholder={selectedActions.includes("rewrite") ? "Paste the draft you wrote…" : "Paste the specific message you want Beckett to focus on…"} className="block w-full resize-y rounded-card border border-border px-4 py-3 pb-12 text-sm font-normal leading-relaxed outline-none focus:border-primary" /><button type="button" disabled aria-label="Attach a screenshot" title="Screenshot attachments are coming during beta" className="absolute bottom-3 left-3 flex h-8 w-8 cursor-not-allowed items-center justify-center rounded-full border border-border bg-bg text-base text-ink-light" aria-disabled="true">📎</button></span></label><div className="mt-1 text-right text-xs text-ink-light">{text.length.toLocaleString()} / 5,000</div>
