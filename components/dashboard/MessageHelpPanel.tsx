  ...
  return (
    <div className="mx-auto max-w-5xl pb-12">
      ...
      {response.length === 0 ? <>
        ...
        <div className="rounded-card border border-primary/20 bg-primary-light/25 p-5 shadow-sm sm:p-6"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="text-xs font-medium uppercase tracking-wide text-primary">Your original message</p><p className={`mt-3 whitespace-pre-wrap text-sm leading-7 text-ink ${!originalExpanded && submittedText.length > 180 ? "line-clamp-2" : ""}`}>{submittedText}</p>{submittedText.length > 180 && <button type="button" onClick={() => setOriginalExpanded((current) => !current)} aria-expanded={originalExpanded} className="mt-2 text-sm font-medium text-primary hover:underline">{originalExpanded ? "Show less ↑" : "Show more ↓"}</button>}</div><button type="button" onClick={startOver} className="shrink-0 rounded-pill border border-border bg-white/70 px-3 py-2 text-xs font-medium text-ink-mid hover:border-primary hover:text-primary">Edit request</button></div>{submittedContext && <div className="mt-5 border-t border-primary/20 pt-4"><p className="text-xs font-medium uppercase tracking-wide text-primary/75">Conversation context</p><p className={`mt-2 whitespace-pre-wrap text-sm leading-7 text-ink-mid ${!contextExpanded && submittedContext.length > 180 ? "line-clamp-2" : ""}`}>{submittedContext}</p>{submittedContext.length > 180 && <button type="button" onClick={() => setContextExpanded((current) => !current)} aria-expanded={contextExpanded} className="mt-2 text-sm font-medium text-primary hover:underline">{contextExpanded ? "Show less ↑" : "Show more ↓"}</button>}</div>}</div>
