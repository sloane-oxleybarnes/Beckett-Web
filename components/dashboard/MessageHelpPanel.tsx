...
function practiceHandoffSituation(message: string) {
  return `I received this message: “${message.trim().slice(0, 3800)}” and want to practice how to respond.`;
}
...
export default function MessageHelpPanel() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (wantsPractice && apiActions.length === 0) {
      sessionStorage.setItem("beckett-practice-prefill", JSON.stringify({ person: person.trim() || "the other person", situation: practiceHandoffSituation(text), goal: goal.trim() || "Practice a clear response to this message.", relationshipContext: context.trim().slice(0, 1000) }));
